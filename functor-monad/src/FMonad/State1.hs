...
joinState :: State1 s (State1 s x) a -> State1 s x a
joinState = State1 . ffmap applied . runState1 . ffmap runState1

cisState :: (forall r. s r -> (s r, x a)) -> State1 s x a
cisState f = State1 $ Curried $ \s op -> let (s', x) = f s in Day s' x op

transState :: (forall r. s r -> (s a, x r)) -> State1 s x a
transState f = State1 $ Curried $ \s op -> let (s', x) = f s in Day s' x (flip op)

instance FFunctor (State1 s) where
    ffmap fg (State1 st) = State1 $ ffmap (ffmap fg) st

instance FMonad (State1 s) where
    fpure = pureState
