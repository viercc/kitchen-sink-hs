...
propMonadUnitL
  :: forall m. (Monad m, forall b. Eq b => Eq (m b))
  => m Var -> Bool
...
checkMonad
  ...
  => Enum1 m -> Bool
checkMonad enum1 =
  and [ all propMonadUnitL (skolem1 enum1)
...
checkMonadIO enum1 =
  case counterExamples of
    [] -> return True
    (bad:_) -> do putStrLn "Counterexample:"
                  putStrLn bad
                  return False
  where
    ...
    badAssoc =
      [ "mmmx = " ++ show mmmx ++ "\n" ++
        "join . join      $ mmmx = " ++ show mx1 ++ "\n" ++
        "join . fmap join $ mmmx = " ++ show mx2
      | mmmx <- skolem3 enum1
